...
export async function submitQuoteRequest(_prev: QuoteFormState, formData: FormData): Promise<QuoteFormState> {
  ...
  const date = new Intl.DateTimeFormat("en-GB", { timeZone: "Europe/Rome", day: "numeric", month: "short", year: "numeric" }).format(now);
  ...
  const mw = ((r.units * k.unitKw) / 1000).toFixed(1).replace(/\.0$/, "");
  ...
  const inputRows: QuotePdfRow[] = [
    { label: t.power, value: `${fmt(inputs.power)} kW` },
  ...
  const resultRows: QuotePdfRow[] = [
    { label: t.metric_heat, value: `${fmt(r.heat)} kW` },
    { label: t.metric_capacity, value: `${fmt(r.capacity)} kW` },
    { label: t.metric_derated, value: `${fmt(r.effectiveUnitKw)} kW` },
    { label: t.metric_headroom, value: `+${Math.max(0, r.headroomPct)}%` },
    { label: t.hud_footprint, value: `${fmt(r.footprintM2, 1)} m²` },
    { label: t.hud_water, value: `${fmt(r.waterLpm)} L/min` },
    { label: t.hud_weight, value: `${fmt(r.weightT, 1)} t` },
    { label: t.hud_electrical, value: `${fmt(r.electricalKva)} kVA` },
  ];
  const statusLine = [
    t.status_online,
  ...
  const configLink = `${SITE_URL}/tower-m?${new URLSearchParams({
  ...
  try {
    pdf = await buildQuotePdf({
      ...
      buildLine: `${config} · ${fmt(r.footprintM2, 1)} m² ${t.footprint_word}`,
      statusLine,
  ...
  const kiwiLead = {
    ...
    message: kiwiMessage,
    source: "enfrio.it/tower-m · configuratore M Tower",
    metadata: {
      type: "mtower_quote",
      ref,
      company: values.company,

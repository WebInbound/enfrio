...
export async function metadata(lang: Lang): Promise<Metadata> {
  const { seo } = await getContent(HOME, lang);
  return { title: seo.title, description: seo.description };
}

export default async function HomePage({ lang }: { lang: Lang }) {
  const [c, g, e, ge] = await Promise.all([getContent(HOME, lang), getContent(GLOBAL, lang), getEdit(HOME, lang), getEdit(GLOBAL, lang)]);
    ...
    <SiteShell lang={lang} active="home">
      <section className="page-hero split">
        <div className="page-hero-text reveal">
          ...
          <div className="btn-row">
            <Link className="btn solid magnetic" href={localePath(lang, "/contact")} {...e.hero.cta_primary}>
              {hero.cta_primary}
            </Link>
            <Link className="btn ghost" href={localePath(lang, "/solutions")} {...e.hero.cta_secondary}>
              {hero.cta_secondary}
            </Link>
          </div>
        </div>
      ...
      <section className="section">
        <div className="mtower-spotlight reveal">
          <div className="mtower-spotlight-content">
            ...
            <div className="btn-row">
              <Link className="btn solid magnetic" href={localePath(lang, "/tower-m")} {...e.spotlight.cta_primary}>{spotlight.cta_primary}</Link>
              <Link className="btn ghost" href={localePath(lang, "/tower-m#mtower-sizer")} {...e.spotlight.cta_secondary}>{spotlight.cta_secondary}</Link>
            </div>
          </div>
      ...
      <section className="section cta reveal">
        ...
        <div className="btn-row" style={{ justifyContent: "center" }}>
          <Link className="btn solid magnetic" href={localePath(lang, "/contact")} {...e.cta.cta_primary}>
            {cta.cta_primary}
          </Link>
          <Link className="btn ghost" href={localePath(lang, "/projects")} {...e.cta.cta_secondary}>
            {cta.cta_secondary}
          </Link>
        </div>
      </section>

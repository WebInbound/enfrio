...
export async function metadata(lang: Lang): Promise<Metadata> {
  const { seo } = await getContent(SOLUTIONS, lang);
  return pageSeo("/solutions", seo, lang);
}

export default async function SolutionsPage({ lang }: { lang: Lang }) {
  const { hero, offer, execution, variants, cta } = await getContent(SOLUTIONS, lang);

  const e = await getEdit(SOLUTIONS, lang);

  return (
    <SiteShell lang={lang} active="solutions">
      ...
      <section className="section cta reveal">
        ...
        <div className="btn-row" style={{ justifyContent: "center" }}>
          <Link className="btn solid" href={localePath(lang, "/contact")} {...e.cta.button}>
            {cta.button}
          </Link>
        </div>
      </section>

...
export async function metadata(lang: Lang): Promise<Metadata> {
  const { seo } = await getContent(LEGAL, lang);
  return pageSeo("/legal", seo, lang);
}

export default async function LegalPage({ lang }: { lang: Lang }) {
  const [c, g] = await Promise.all([getContent(LEGAL, lang), getGlobal(lang)]);
  ...
  const e = await getEdit(LEGAL, lang);

  return (
    <SiteShell lang={lang} active="legal">

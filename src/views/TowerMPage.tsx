...
export async function metadata(lang: Lang): Promise<Metadata> {
  const { seo } = await getContent(TOWER_M, lang);
  return pageSeo("/tower-m", seo, lang);
}
...
export default async function TowerMPage({ lang }: { lang: Lang }) {
  const [c, sizer, g, quote, form] = await Promise.all([
    getContent(TOWER_M, lang),
  ...
  const [e, ec, sc, qe, fe, editing] = await Promise.all([
    getEdit(TOWER_M, lang),
    getEditForClient(TOWER_M, lang),
    getEditForClient(SIZER, lang),
    getEditForClient(QUOTE, lang),
    getEditForClient(CONTACT_FORM, lang),
    isEditing(),
  ]);

  return (
    <SiteShell lang={lang} active="tower-m">
        ...
        <div className="mtower-outro-grid">
          ...
          <div className="mtower-outro-content reveal">
            ...
            <div className="btn-row">
              <Link className="btn solid magnetic" href={localePath(lang, "/contact?subject=M+Tower+Inquiry")} {...e.outro.cta}>
                {outro.cta}
              </Link>

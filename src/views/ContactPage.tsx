...
export async function metadata(lang: Lang): Promise<Metadata> {
  const { seo } = await getContent(CONTACT, lang);
  return pageSeo("/contact", seo, lang);
}

export default async function ContactPage({ lang }: { lang: Lang }) {
  const [{ hero, inquiry, side }, form, g] = await Promise.all([
    getContent(CONTACT, lang),
    getContent(CONTACT_FORM, lang),
    getGlobal(lang),
  ]);
  const co = companyInfo(g);

  const [e, fe] = await Promise.all([getEdit(CONTACT, lang), getEditForClient(CONTACT_FORM, lang)]);

  return (
    <SiteShell lang={lang} active="contact">
        ...
        <div className="contact-layout">
          ...
          <aside className="contact-side reveal">
            <article className="contact-side-card">
              ...
              <p><a href={`mailto:${co.email}`} aria-label={fill(g.a11y.send_email, { email: co.email })}>{co.email}</a></p>
            </article>
            <article className="contact-side-card">
